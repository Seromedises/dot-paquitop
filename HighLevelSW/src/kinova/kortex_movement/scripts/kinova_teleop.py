...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        rospy.Subscriber("/extract_tablet", Bool, GripperIO)
        # external_forces_fb = rospy.wait_for_message("/my_gen3_lite/base_feedback", BaseCyclic_Feedback)
        moveArm = rospy.wait_for_message("/velTwistArm", Twist)
        
        # line = str(external_forces_fb.base.tool_external_wrench_force_x)+"\n"+str(external_forces_fb.base.tool_external_wrench_force_y)+"\n"+str(external_forces_fb.base.tool_external_wrench_torque_z)+"\n\n"
        # print(line)
        if abs(moveArm.linear.x) < 0.05:
            moveArm.linear.x = 0.0
        if abs(moveArm.linear.y) < 0.05:
            moveArm.linear.y = 0.0
        if abs(moveArm.linear.z) < 0.05:
            moveArm.linear.z = 0.0
        if abs(moveArm.angular.x) < 0.06:
            moveArm.angular.x = 0.0
        if abs(moveArm.angular.y) < 0.06:
            moveArm.angular.y = 0.0
        ...
        msg.twist.linear_z =  moveArm.linear.z
        msg.twist.angular_x=  moveArm.angular.x
        msg.twist.angular_y =  moveArm.angular.y
        msg.twist.angular_z =  moveArm.angular.z
        msg.duration = 1/50
